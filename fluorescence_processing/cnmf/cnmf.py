...
from caiman.source_extraction.cnmf.initialization import initialize_components, compute_W, initialize_components_fixed_A
...
class CNMF(object):
    ...
    def fit_v2(self, images,  indices=(slice(None), slice(None))) -> None:
            """
                    ...
                    Args:
                        ...
                        A_fixed_mode: fixed shape of neurons mode
            ...
            if self.params.get('patch', 'rf') is None and (is_sliced or 'ndarray' in str(type(images))):
                ...
                self.dview = None
                logger.info(
                    "Parallel processing in a single patch is not available for data that is in memory or sliced")
            ...
            #     API (if we go with execution contexts, definitely there)
            if self.params.get('preprocess', 'n_pixels_per_process') is None:
                avail_memory_per_process = psutil.virtual_memory()[1] / 2. ** 30 / self.params.get('patch',
                                                                                                   'n_processes')
                mem_per_pix = 3.6977678498329843e-09
                npx_per_proc = int(avail_memory_per_process / 8. / mem_per_pix / T)
                npx_per_proc = int(
                    np.minimum(npx_per_proc, np.prod(self.dims) // self.params.get('patch', 'n_processes')))
                self.params.set('preprocess', {'n_pixels_per_process': npx_per_proc})
            ...
            if self.params.get('patch', 'rf') is None:  # no patches
                ...
                if self.estimates.A is None:
                    ...
                    self.initialize_with_A_fixed(Y)
                ...
                if self.params.get('patch', 'only_init'):  # only return values after initialization
                    ...
                    if self.remove_very_bad_comps:
                        ...
                        logger.info(
                            f"Keeping {len(idx_components)} components and discarding {len(idx_components_bad)} components")
                        self.estimates.C = self.estimates.C[idx_components]
                ...
                if (self.estimates.b is not None) and (len(list(np.where(~self.estimates.b.any(axis=0))[
                                                                    0])) > 0):  # If any of the background ended up completely empty
                    raise Exception(
                        "After run_CNMF_patches(), one or more of the background components is empty. Please restart analysis with init/nb set to a lower value")
    ...
    def initialize_with_A_fixed(self, Y, **kwargs) -> None:
        ...
        if (self.params.get('init', 'method_init') == 'corr_pnr' and
                self.params.get('init', 'ring_size_factor') is not None):
            A_temp = self.estimates.A.copy()
            estim.A, estim.C, estim.b, estim.f, estim.center, \
                extra_1p = initialize_components_fixed_A(
                Y, A=A_temp ,sn=estim.sn, options_total=self.params.to_dict(),
                **self.params.get_group('init'))
            try:
                estim.S, estim.bl, estim.c1, estim.neurons_sn, \
                    estim.g, estim.YrA, estim.lam = extra_1p
