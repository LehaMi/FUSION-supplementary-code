...
def update_temporal_components_fixed_A(Y, A, b, Cin, fin, bl=None, c1=None, g=None, sn=None, nb=1, ITER=2, block_size_temp=5000, num_blocks_per_run_temp=20, debug=False, dview=None, **kwargs):

    """
    固定 A 和 b，仅更新时间组件 C 和 f。
    完全保留原版逻辑，唯一区别是移除了“删除空组件”的代码块。
    """
    logger = logging.getLogger("caiman")
    if 'p' not in kwargs or kwargs['p'] is None:
        raise Exception("You have to provide a value for p")
    ...
    if b is not None:
        if b.ndim == 1:
            b = b.reshape(-1, 1)
        elif b.shape[0] != d and b.shape[1] == d:
            b = b.T
        nb = b.shape[1]

    if bl is None: bl = np.repeat(None, nr)
    if c1 is None: c1 = np.repeat(None, nr)
    if g is None:  g = np.repeat(None, nr)
    if sn is None: sn = np.repeat(None, nr)

    # 2. 数据准备
    # 使用 scipy.sparse.issparse 检查
    if not scipy.sparse.issparse(A):
        A = csc_matrix(A)

    A_tot = scipy.sparse.hstack((A, b)).tocsc()
    S = np.zeros(Cin.shape)
    Cin_tot = np.vstack((Cin, fin))
    C_tot = Cin_tot.copy()

    nA = np.ravel(A_tot.power(2).sum(axis=0)) + np.finfo(np.float32).eps

    logger.info('Generating residuals (Projection)...')

    # 3. 计算 YA 和 AA
    if 'memmap' in str(type(Y)):
        bl_siz1 = d // (np.maximum(num_blocks_per_run_temp - 1, 1))
        avail_mem = psutil.virtual_memory().available
        bl_siz2 = int(avail_mem/(num_blocks_per_run_temp + 1) - 4*A_tot.nnz) // int(4*T)
        block_size = min(bl_siz1, bl_siz2) if bl_siz2 > 0 else bl_siz1

        # 这里需要 explicit import diags, 已在顶部添加
        YA = caiman.mmapping.parallel_dot_product(Y, A_tot.tocsr(), dview=dview,
                                                  block_size=block_size,
                                                  transpose=True,
                                                  num_blocks_per_run=num_blocks_per_run_temp) * diags(1. / nA)
    else:
        YA = (A_tot.T.dot(Y).T) * diags(1. / nA)

    AA = ((A_tot.T.dot(A_tot)) * diags(1. / nA)).tocsr()
    ...
    logger.info(f"Entering Deconvolution. Shape: {C_tot.shape}")
    ...
    # -----------------------------------------------------------
    # 【已移除】删除空组件的代码块，确保 A 形状不变
    # -----------------------------------------------------------

    # 5. 拆分结果
    b_out = A_tot[:, nr:].toarray()
    A_out = csc_matrix(A_tot[:, :nr])
    f_out = C_tot[nr:, :]
    C_out = C_tot[:nr, :]
    YrA_out = np.array(YrA[:, :nr]).T

    return C_out, A_out, b_out, f_out, S, bl, c1, sn, g, YrA_out, lam
