...
def initialize_components_fixed_A(Y, A, K=30, gSig=[5, 5], gSiz=None, ssub=1, tsub=1, nIter=5, maxIter=5, nb=1,
                          kernel=None, use_hals=True, normalize_init=True, img=None, method_init='greedy_roi',
                          max_iter_snmf=500, alpha_snmf=0.5, sigma_smooth_snmf=(.5, .5, .5),
                          perc_baseline_snmf=20, options_local_NMF=None, rolling_sum=False,
                          rolling_length=100, sn=None, options_total=None,
    ...
    if method == 'greedy_roi':
        ...
        Ain, Cin, _, b_in, f_in, extra_1p = greedyROI_corr_fixed_A(
            Y,Y_ds,A_fixed=A, max_number=K, gSiz=gSiz[0], gSig=gSig[0], min_corr=min_corr, min_pnr=min_pnr,
            ring_size_factor=ring_size_factor, center_psf=center_psf, options=options_total,
            sn=sn, nb=nb, ssub=ssub, ssub_B=ssub_B, init_iter=init_iter, seed_method=seed_method)
...
@profile
def greedyROI_corr_fixed_A(Y, Y_ds, A_fixed, max_number=None, gSiz=None, gSig=None, center_psf=True,
                   min_corr=None, min_pnr=None, seed_method='auto',
                   min_pixel=3, bd=0, thresh_init=2, ring_size_factor=None, nb=1, options=None,
                   sn=None, save_video=False, video_name='initialization.mp4', ssub=1,
                   ssub_B=2, init_iter=2):
    ...
    A, C, _, _, center = init_neurons_corr_pnr_fixed_A(
        ...
        center_psf=center_psf, deconvolve_options=o,
        swap_dim=True)
    ...
    if ring_size_factor is not None:
        ...
        logger.info('Updating spatial components')
        # A, _, C, _ = caiman.source_extraction.cnmf.spatial.update_spatial_components_fixed_A(
        #     B, C=C, f=np.zeros((0, total_frames), np.float32), A_in=A,
        #     sn=np.sqrt(downscale((sn**2).reshape(dims, order='F'),
        #                          tuple([ssub] * len(dims))).ravel() / tsub) / ssub,
        #     b_in=np.zeros((d1 * d2, 0), np.float32),
        ...
        C = C.astype(np.float32)
        logger.info('Updating spatial components')
        A, _, C, _ = caiman.source_extraction.cnmf.spatial.update_spatial_components_fixed(
            B, C=C, f=np.zeros((0, total_frames), np.float32), A_in=A,
            sn=np.sqrt(downscale((sn**2).reshape(dims, order='F'),
                                 tuple([ssub] * len(dims))).ravel() / tsub) / ssub,
            b_in=np.zeros((d1 * d2, 0), np.float32),
        ...
        logger.info('Updating temporal components')
        C, A, b__, f__, S, bl, c1, neurons_sn, g1, YrA, lam__ = \
            caiman.source_extraction.cnmf.temporal.update_temporal_components_fixed_A(
                B, spr.csc_matrix(A, dtype=np.float32),
                np.zeros((np.prod(dims), 0), np.float32), C, np.zeros((0, T), np.float32),
                dview=None, bl=None, c1=None, sn=None, g=None, **options['temporal'])
...
def init_neurons_corr_pnr_fixed_A(data, A_fixed, gSiz=15, gSig=None,
                                  center_psf=True,
                                  deconvolve_options=None, swap_dim=True):
    """
    专门用于基于 A_fixed 的初始化。
    不进行任何搜索，不计算全图 Correlation/PNR。

    Args:
        data: 原始视频数据 (d1, d2, T) or (T, d1, d2)
        A_fixed: 稀疏矩阵或数组，已知的神经元形状
        gSiz: 剪裁窗口的半径（必须包含背景环）
        gSig: 仅用于空间滤波（可选，为了让数据更平滑）
    """
    logger = logging.getLogger("caiman")
    ...
    if swap_dim:
        d1, d2, total_frames = data.shape
        data_raw = np.transpose(data, [2, 0, 1])  # 变为 (T, d1, d2) 用于滤波方便
    else:
        total_frames, d1, d2 = data.shape
        data_raw = data

    # 2. 空间滤波 (Spatial Filtering) - 仅用于去噪，不改变形状
    data_filtered = data_raw.copy()
    if gSig:
        if not isinstance(gSig, list):
            gSig = [gSig, gSig]
    ...
    # 统一把 data 转为 (d1, d2, T) 方便后续切片
    data_raw = np.transpose(data_raw, [1, 2, 0])  # (d1, d2, T)
    data_filtered = np.transpose(data_filtered, [1, 2, 0])  # (d1, d2, T)

    # 3. 准备结果容器
    if not spr.issparse(A_fixed):
        A_fixed = spr.csc_matrix(A_fixed)

    num_neurons = A_fixed.shape[1]
    logger.info(f"Fixed Initialization: Processing {num_neurons} components...")

    Ain = np.zeros(shape=(num_neurons, d1, d2), dtype=np.float32)
    Cin = np.zeros(shape=(num_neurons, total_frames), dtype=np.float32)
    Sin = np.zeros(shape=(num_neurons, total_frames), dtype=np.float32)
    Cin_raw = np.zeros(shape=(num_neurons, total_frames), dtype=np.float32)
    center = np.zeros(shape=(2, num_neurons))
    ...
    for i in range(num_neurons):
        # 4.1 获取形状
        ai_full = A_fixed[:, i].toarray().reshape((d1, d2), order='F')

        # === 修改点 A：使用权重最大值的位置作为中心 ===
        ind_max = np.argmax(ai_full)
        # unflatten index to (row, col)
        r_ctr, c_ctr = np.unravel_index(ind_max, (d1, d2))

        # 检查是否为空 (如果最大值也是0，说明全是0)
        if ai_full[r_ctr, c_ctr] == 0:
            continue

        # gSiz 必须足够大
        r_min = max(0, int(r_ctr - gSiz))
        r_max = min(d1, int(r_ctr + gSiz + 1))
        c_min = max(0, int(c_ctr - gSiz))
        c_max = min(d2, int(c_ctr + gSiz + 1))
        ...
        # 4.3 剪裁数据
        # 这里的 reshape 会把数据变成 (Pixels, T)，但 extract_ac 通常需要 (T, Pixels)
        # 所以我们在 reshape 后加一个 .T 进行转置
        # 使用 filtered 数据提取 trace 会更干净，但如果想保留原始噪声特性可以用 data_raw
        data_box = data_filtered[r_min:r_max, c_min:c_max, :].reshape(-1, total_frames).T  # shape: (T, Pixels)

        # 剪裁 ai
        ai_box = ai_full[r_min:r_max, c_min:c_max]

        # 4.4 调用 extract_ac_fixed_A
        # 传入裁剪后的数据、尺寸和固定的形状 ai_box
        [ai_out, ci_out, success] = extract_ac_fixed_A(data_box, patch_dims, ai_box)

        if success:
            # 4.5 存结果
            center[:, valid_count] = [c_ctr, r_ctr]
            Ain[valid_count, r_min:r_max, c_min:c_max] = ai_out
            Cin_raw[valid_count] = ci_out

            # 4.6 去卷积 (Deconvolution)
            if deconvolve_options and deconvolve_options.get('p', 0) > 0:
                try:
                    # 需要从 caiman 导入 constrained_foopsi
                    from caiman.source_extraction.cnmf.deconvolution import constrained_foopsi
                    ci, _, _, _, _, si, _ = constrained_foopsi(ci_out, **deconvolve_options)
                    Cin[valid_count] = ci
                    Sin[valid_count] = si
                except Exception:
                    Cin[valid_count] = ci_out
            else:
                ci = ci_out.copy()
                ci[ci < 0] = 0
                Cin[valid_count] = ci
    ...
    # 5. 整理最终输出
    A = np.reshape(Ain[:valid_count], (-1, d1 * d2), order='F').transpose()
    C = Cin[:valid_count]
    C_raw = Cin_raw[:valid_count]
    S = Sin[:valid_count]
    center = center[:, :valid_count]

    logger.info(f"Fixed Initialization complete. {valid_count} components extracted.")
    return A, C, C_raw, S, center
...
def extract_ac_fixed_A(data_raw, patch_dims, A_fixed):
    """
    固定 A_fixed 形状，利用空间权重提取时间序列 C。
    采用加权投影 (Weighted Projection) 方式，而非简单的阈值平均。

    Args:
        data_raw: 裁剪后的视频数据，形状应为 (T, Pixels)
        patch_dims: (height, width) 图像块尺寸
        A_fixed: 当前神经元在 patch 内的固定形状 (2D array or 1D array)
    """
    ...
    # 找到非零权重的索引 (为了减少计算量，只计算有权重的部分)
    # 这里可以使用 > 0，因为 A_fixed 应该已经去除了背景噪声
    idx_nonzero = ai_flat > 0

    if not np.any(idx_nonzero):
        return None, None, False

    # 提取有效的权重向量 w 和 对应的数据 Y
    w = ai_flat[idx_nonzero]  # shape: (N_pixels,)
    Y = data_raw[:, idx_nonzero]  # shape: (T, N_pixels)

    # 2. 提取时间活性 (Weighted Projection / Least Squares)
    # 公式: C = (Y · w) / (w · w)
    # 这相当于在空间上做匹配滤波，信噪比最优
    ...
    # 分子: 数据在权重方向上的投影 (向量, 长度 T)
    # Y (T, N) dot w (N,) -> (T,)
    ci = np.dot(Y, w) / w_sq_sum

    # 3. 去除基线 (Remove baseline)
    # 使用 Welch 方法估算噪声水平
    sn = get_noise_welch(ci)
    y_diff = np.concatenate([[-1], np.diff(ci)])
    ...
    # 估算基线 b
    # 选取变化率较小且非负的时间点来估算底噪
    mask_b = (y_diff >= 0) * (y_diff < sn)
    if np.any(mask_b):
        b = np.median(ci[mask_b])
    else:
        b = np.min(ci)  # Fallback
    ...
    # 返回结果：ai (原形状), ci (加权提取的时间序列), True
    return ai.reshape(patch_dims), ci, True
