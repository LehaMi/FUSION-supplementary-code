...
def update_spatial_components_fixed_A(Y, C=None, f=None, A_in=None, sn=None, dims=None,
                                      b_in=None, nb=1, update_background_components=True,
                                      dview=None, num_blocks_per_run_spat=20, **kwargs):
    """
    Update spatial background b given fixed spatial components A.
    Designed for 'fit_again' to keep A fixed.
    For nb=0, this effectively just returns inputs unchanged.
    """
    logger = logging.getLogger("caiman")
    logger.info('Updating Spatial Components (Fixed A mode)')

    if A_in is None: raise ValueError("A_in required")
    A = A_in if scipy.sparse.issparse(A_in) else csc_matrix(A_in)

    if C is None: raise ValueError("C required")
    C = np.array(C)

    if f is None:
        f = np.zeros((0, C.shape[1]), dtype=np.float32)
    else:
        f = np.array(f)

    if b_in is None:
        b = np.zeros((Y.shape[0], max(0, nb)), dtype=np.float32)
    else:
        b = b_in

    # Only update background if nb > 0
    if update_background_components and nb > 0:
        logger.info("Computing residuals for background update")
        # Calc Residuals Y_res = Y*f.T - A*C*f.T
        if 'memmap' in str(type(Y)):
            bl_siz1 = Y.shape[0] // (num_blocks_per_run_spat - 1)
            avail = psutil.virtual_memory().available
            bl_siz2 = avail // (4 * Y.shape[-1] * (num_blocks_per_run_spat + 1))
            block_size = min(bl_siz1, bl_siz2)
            if block_size < 1000: block_size = 1000

            Yf = caiman.mmapping.parallel_dot_product(Y, f.T, dview=dview,
                                                      block_size=block_size,
                                                      num_blocks_per_run=num_blocks_per_run_spat)
        else:
            Yf = np.dot(Y, f.T)

        Y_resf = Yf - A.dot(C.dot(f.T))

        # Update b using HALS or simple regression
        if np.sum(b) == 0:
            ffT = f.dot(f.T)
            ffT[np.diag_indices_from(ffT)] += 1e-6
            try:
                b = np.fmax(np.linalg.solve(ffT, Y_resf.T), 0).T
            except np.linalg.LinAlgError:
                b = np.fmax(np.linalg.lstsq(ffT, Y_resf.T, rcond=None)[0], 0).T
        else:
            # HALS update
            # We assume dense b update for simplicity if b is passed as dense
            ind_b = [np.arange(b.shape[0]) for _ in range(b.shape[1])]
            b = HALS4shape_bckgrnd(Y_resf, b, f, ind_b)
